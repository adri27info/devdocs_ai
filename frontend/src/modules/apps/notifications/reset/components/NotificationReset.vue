    ...
    >
      <div class="w-full mb-8 flex flex-col items-center justify-center gap-2 text-center">
        <div class="w-full mb-5 flex flex-col items-center justify-center gap-2 text-center">
          <h1 class="text-xl font-semibold mb-2">Reset cache notifications</h1>
          <p class="text-base">
            {{
              storeNotification.notificationsFactory.list.length
                ? "Here's an overview of your reset cache notifications"
                : 'Reset cache notifications cannot be displayed'
            }}
          </p>
        </div>
                ...
                <button
                  ...
                  @click="
                    resetCache(
                      ...
                      notification.reset_reason,
                    )
                  "
                  :class="{ disabled: storeCore.btnDisablers.resetBtn }"
                >
                  Reset cache key
                </button>

                <Loader :loading="resetLoaders[notification.id]" />
...
import { useNotificationReset } from '@/modules/apps/notifications/reset/composables/useNotificationReset';
...
const { resetLoaders, resetCache } = useNotificationReset();

const { isOpen, selectedNotificationId, deleteNotification, openModal, closeModal } =
  useNotificationFetcher('RESET');
</script>
