    ...
    >
      <div class="w-full mb-8 flex flex-col items-center justify-center gap-2 text-center">
        <div class="w-full mb-5 flex flex-col items-center justify-center gap-2 text-center">
          <h1 class="text-xl font-semibold mb-2">Action required notifications</h1>
          <p class="text-base">
            {{
              storeNotification.notificationsFactory.list.length
                ? "Here's an overview of your action required notifications"
                : 'Action required notifications cannot be displayed'
            }}
          </p>
        </div>
        ...
        >
          ...
          >
            <div
              class="w-full flex flex-col xl:flex-row items-center justify-center xl:justify-around gap-5 rounded-2xl border-2 p-2 min-h-36"
            >
              ...
              <div class="flex flex-col justify-center items-center gap-2">
                <input
                  ...
                  v-model="notificationActionRequiredFactory[notification.id].invitationCode"
                  @input="validateInvitationCode(notification.id)"
                />

                <p
                  v-if="notificationActionRequiredFactory[notification.id].invitationCodeError"
                  class="w-full text-error text-center font-bold"
                >
                  {{ notificationActionRequiredFactory[notification.id].invitationCodeError }}
                </p>

                <Loader :loading="projectConfirmLoaders[notification.id]" />

                <button
                  type="button"
                  class="p-2 btn btn-dark mt-4"
                  @click="confirmProjectInvitationCode(notification.id)"
                  :class="{ disabled: storeCore.btnDisablers.projectConfirmBtn }"
                >
...
import { useNotificationActionRequired } from '@/modules/apps/notifications/action-required/composables/useNotificationActionRequired';
...
const { isOpen, selectedNotificationId, deleteNotification, openModal, closeModal } =
  useNotificationFetcher('ACTION_REQUIRED');
